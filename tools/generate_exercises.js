...
lines.forEach(line => {
    ...
    if (line.includes('(') && line.endsWith(')')) {
        ...
        const exName = line.split(' - ')[0].trim();
        const exId = idify(exName);
        if (currentCategory) {
            exercisesByCat[currentCategory].push({
                id: exId,
                name: exName,
                sets: currentCategory === 'cardio' ? 1 : 3,
                reps: currentCategory === 'cardio' ? '20 min' : '10-12',
                rest: currentCategory === 'cardio' ? 0 : 60,
                instructions: ''
            });
        }
    }
